...
import { 
  ...
  Loader2, BrainCircuit, Clock, Info, ShieldCheck, Download, 
  Github, Instagram, Globe 
} from "lucide-react";
...
export default function Home() {
  ...
  const generatePDF = () => {
    ...
    // ... (Lógica do PDF preservada)
    doc.save(`Relatorio_Anubis_Final.pdf`);
  };
    ...
    <div className="flex flex-col min-h-screen bg-[#FAF8F5] text-left animate-in fade-in duration-700">
      <div className="flex-grow p-4 md:px-12 py-8 flex flex-col gap-8 pb-40">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <SummaryCard title="Contas a Pagar" value={aPagar} color="bg-[#FFF1E6]" textColor="text-[#E67E22]" icon={<AlertCircle size={18}/>} />
        </div>
          ...
          <table className="w-full text-left font-bold">
            <thead>
              <tr className="border-b border-[#F1E7E4] text-[#A17C7C] text-[10px] uppercase tracking-[0.2em] font-black italic">
                <th className="px-10 py-6 italic">Descrição & Rastreabilidade</th>
                  ...
                  <td className="px-10 py-5">
                    <div className="flex flex-col">
                      ...
                      <span className="text-[9px] font-black text-[#A17C7C] uppercase tracking-widest mt-1.5 opacity-60">
                        {new Date(t.created_at).toLocaleDateString('pt-BR')} | {t.sector} {t.invoice_number && `| NF: ${t.invoice_number}`}
                      </span>
                    </div>
                  </td>
      ...
      {/* FOOTER ANUBIS TECH PERSONALIZADO [cite: 2026-02-13] */}
      <footer className="fixed bottom-0 left-0 right-0 bg-[#FAF8F5]/90 backdrop-blur-xl py-8 px-12 flex flex-col md:flex-row justify-between items-center border-t border-[#E4D5D1] z-[100] shadow-[0_-20px_50px_rgba(0,0,0,0.05)]">
        <div className="flex items-center gap-6">
          <div className="bg-[#4A3737] p-3 rounded-2xl shadow-xl">
            <ShieldCheck size={22} className="text-[#D4A5A5]" />
          </div>
          <div className="text-left">
            <p className="text-[11px] font-black text-black uppercase tracking-[0.3em]">
              © 2026 <span className="text-[#6B4F4F]">Anubis Tech</span>
            </p>
            <p className="text-[9px] font-bold text-black uppercase tracking-widest mt-1">
              CNPJ: 42.804.763/0001-35 | Todos os direitos reservados.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-10 mt-6 md:mt-0">
          <div className="flex gap-6">
            <a href="https://github.com/fernando-hugo" target="_blank" rel="noopener noreferrer" className="text-black hover:text-[#D4A5A5] transition-all transform hover:scale-110">
              <Github size={20} />
            </a>
            <a href="https://www.instagram.com/anubis.tec/" target="_blank" rel="noopener noreferrer" className="text-black hover:text-[#D4A5A5] transition-all transform hover:scale-110">
              <Instagram size={20} />
            </a>
            <a href="#" className="text-black hover:text-[#D4A5A5] transition-all transform hover:scale-110">
              <Globe size={20} />
            </a>
          </div>
          <div className="h-10 w-px bg-[#E4D5D1] hidden md:block"></div>
          <div className="text-right">
            <p className="text-[9px] font-black text-[#A17C7C] uppercase tracking-widest mb-1">System Health</p>
            <p className="text-[10px] font-bold text-[#10B981] uppercase flex items-center gap-2 italic">
...
function SummaryCard({ title, value, color, textColor, icon }: any) {
  return (
    <div className={`${color} p-8 rounded-[35px] border border-white shadow-lg flex justify-between items-start text-left hover:shadow-2xl transition-all duration-300`}>
      <div className="text-left">
        <p className={`text-[10px] uppercase font-black ${textColor} opacity-60 tracking-widest`}>{title}</p>
        <p className={`text-2xl font-black ${textColor} tracking-tighter mt-1`}>{value.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</p>
      </div>
