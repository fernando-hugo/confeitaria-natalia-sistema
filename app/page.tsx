...
export default function Home() {
  ...
  const aPagar = transactions
    .filter((t) => !t.paid && Number(t.amount) < 0)
    .reduce((acc, t) => acc + Math.abs(Number(t.amount || 0)), 0);

  return (
    <div className="flex flex-col min-h-screen bg-[#FAF8F5] text-left">
      <div className="flex-grow p-4 md:px-12 py-8 flex flex-col gap-8 pb-48">
                ...
                {loading ? (
                  <tr>
                    <td colSpan={4} className="py-20 text-center">
                      <Loader2 className="animate-spin mx-auto text-[#D4A5A5]" size={32} />
                    </td>
                  </tr>
                ) : (
                  transactions.map((t) => (
                    <tr key={t.id} className="hover:bg-[#FAF8F5]/80 transition-colors">
                      <td className="px-8 py-5">
                        <div className="flex flex-col">
                          <span className="text-sm font-black text-[#4A3737] uppercase italic leading-tight">
                            {t.description}
                          </span>
                          <div className="flex items-center gap-2 mt-2">
                            <span className="text-[8px] font-black bg-[#F1E7E4] px-2 py-0.5 rounded text-[#A17C7C] uppercase">
                              {t.sector}
                            </span>
                      ...
                      <td className={`px-8 py-5 text-center text-sm font-black ${Number(t.amount) > 0 ? "text-[#1B806A]" : "text-[#D14343]"}`}>
                        {Math.abs(Number(t.amount || 0)).toLocaleString("pt-BR", {
                          style: "currency",
                          currency: "BRL",
                        })}
                      </td>
                      <td className="px-8 py-5 text-center">
                        <span className={`text-[8px] px-3 py-1.5 rounded-full font-black uppercase tracking-widest border ${
                          t.paid
      ...
      <footer className="fixed bottom-4 left-1/2 -translate-x-1/2 w-[95%] max-w-6xl bg-white/60 backdrop-blur-md border border-white/40 py-4 px-8 rounded-2xl shadow-lg z-[100] flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="flex items-center gap-6">
          <a href="https://github.com/fernando-hugo" target="_blank" rel="noopener noreferrer" className="text-[#6B4F4F] hover:text-[#D4A5A5] transition-colors">
            <Github size={16} />
          </a>
          <a href="https://www.instagram.com/anubis.tec/" target="_blank" rel="noopener noreferrer" className="text-[#6B4F4F] hover:text-[#D4A5A5] transition-colors">
            <Instagram size={16} />
          </a>
...
function SummaryCard({ title, value, color, textColor, icon }: any) {
  return (
    <div className={`${color} p-5 rounded-[25px] border border-white shadow-sm flex justify-between items-start`}>
      <div>
        <p className={`text-[8px] uppercase font-black ${textColor} opacity-60 tracking-[0.2em]`}>
          {title}
        </p>
